...
class MessageConverter:
    ...
    '''
        ...
        @endcode
    '''
    def MCTL(f_vel=0.0,f_angle=0.0, f_cangle=0.0):
        if type(f_vel==float) and type(f_angle==float) and type(f_cangle==float) and f_cangle != 0.00:
            return "#MCTL:%.2f"%f_vel+";%.2f"%f_angle+";%.2f"%f_cangle+";;\r\n"
    ...
    def USEN(activate=True):
        l_value=0
        if activate:
            l_value=1
        return '#USEN:%d;'%l_value+';\r\n';
...
class SerialHandler:
    ...
    def __init__(self,f_device_File='/dev/ttyACM0',f_history_file='historyFile.txt'):
        self.serialCon=serial.Serial(f_device_File,256000,timeout=1)
    ...
    def sendMove(self,f_vel,f_angle, f_cangle = 0.0):
        str_msg=MessageConverter.MCTL(f_vel,f_angle,f_cangle)
        if str_msg!="":
            self.send(str_msg)
            return True
        else:
    ...
    def activateSensorPublisher(self, activate=True):
        str_msg=MessageConverter.USEN(activate)
        if str_msg!="":
            self.send(str_msg)
            return True
        else:
